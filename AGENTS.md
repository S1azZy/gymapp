...
- RSpec
- SQL-only migrations with explicit `up` / `down` statements
- UUID primary keys generated via PostgreSQL 18 `uuidv7()`
...
- All schema changes must be written as raw SQL inside Rails migrations.
- Do not use Active Record migration DSL such as `create_table`, `change_table`, `add_reference`, or `change`.
...
- Use `CHECK` constraints only for true data-integrity rules that belong to persistence, not for business-level validations better enforced by the application layer.
...
- All primary keys must be UUIDs generated by PostgreSQL 18 `uuidv7()`.
- Foreign keys that reference primary entities must use `uuid`.
- If a model generator is used, generate models with `--skip-migration` and create the SQL migration separately.
...
- Use Haml for view templates instead of ERB.

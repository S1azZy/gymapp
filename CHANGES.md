...
- Synced the roadmap and foundations docs with the actual Phase 2 state, clarified template ownership and `EquipmentType` terminology, and added request coverage for registration rate limiting.
- Started Phase 3.1a with the user-owned workout template domain: SQL schema, models, ownership policy, and baseline factory/model/policy coverage for templates and ordered template exercises.

...
- `structure.sql` as the canonical schema dump
- UUID primary keys generated by PostgreSQL 18 `uuidv7()`
...
### Database migrations
- define schema changes in raw SQL only
- use explicit `up` / `down`
- prefer strict constraints and explicit indexes
- keep business validations in the application layer rather than `CHECK` constraints unless the rule is truly storage-level
- avoid triggers and stored procedures by default
...
- prefer real database integration over mocks and stubs
